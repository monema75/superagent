...
/**
 * Set the trusted certificate fingerprints.
 *
 * @param {String | Strings} fingerprints
 * @return {Request} for chaining
 * @api public
 */

Request.prototype.fingerprints = function (fingerprints) {
...
Request.prototype.request = function () {
  ...
  if (this._fingerprints && this._fingerprints.length) {
    req.on('socket', socket => {
      socket.on('secureConnect', () => {
        var cert = socket.getPeerCertificate();
        var hex = cert.pubkey.toString('hex');

        // need to hash SHA256 
        var hash = crypto.createHash('sha256').update(hex, 'hex').digest('hex');
        // to base64 encoding 
        var pinHash = new Buffer(hash, 'hex').toString('base64');
        ...
        if (self._fingerprints.indexOf(pinHash) === -1 && !socket.isSessionReused()) {
          req.emit('error', { status: 'FINGERPRINT_NOT_MATCHED' });
          return req.abort();
        }
      });
    });
